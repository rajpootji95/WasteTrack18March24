...
class _EcoBlueSubAdminState extends State<EcoBlueSubAdmin> {
  ...
  final subAdminVisibilityController = Get.put(EcoAdminSubAdminVisibilityController());
...
@override
  ...
  Future<void> _fetchData() async {
    ...
    if (!isLoading1 && currentPage < totalPages) {
      setState(() {
        isLoading1 = true;
      });
      await Future.delayed(Duration(seconds: 2));
      setState(() {
        dataList.addAll(List.generate(50, (index) => index + dataList.length));
        currentPage++;
        isLoading1 = false;
      });
    }
  ...
  List<int> dataList = List.generate(50, (index) => index); // Example data list
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder(
        ...
        builder: (controller) => Scaffold(
            ...
            body:controller.ecoModel == null ?  myShimmer() : Container(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ScrollConfiguration(
                        behavior: MyBehavior(),
                        child: controller.ecoModel?.data == null
                            ? myShimmer()
                            : ListView.builder(
                                itemCount:controller.ecoModel!.data.length  + (isLoading1 ? 1 : 0),
                                itemBuilder: (context, index) {
                                   if(index == controller.ecoModel!.data.length){
                                     return Center(child: CircularProgressIndicator());
                                   }else{
                                     //_fetchData();
                                     var ecoBlueSubAdmin = controller.ecoModel?.data[index];
                                     String apiDateString = "${controller.ecoModel?.data[index].createdAt}";
