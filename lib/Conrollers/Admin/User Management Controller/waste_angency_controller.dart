...
class WasteManagementAgencyController extends AppBaseController {
 ...
 //addWasteAgencyApi
  ...
  Future<GetWasteAgencyModel?> getProfileApi() async  {
    var userToken = await SharedPref().getToken();
    var headers = {
      'Authorization': 'Bearer $userToken', // Assuming your token type is 'Bearer'
    };
    Dio dio = Dio();
    try {
      ...
      if (response.statusCode == 200) {
        var result = response.data;
        var finalResult = GetWasteAgencyModel.fromJson(json.decode(result));
